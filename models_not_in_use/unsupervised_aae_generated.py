...
mean_std_dir = latent_space_dir / 'mean2std01'
mean_std_dir.mkdir(exist_ok=True)

sampling_dir = mean_std_dir / 'Sampling'
...
plt.savefig(mean_std_dir / 'Before_training_validation_latentspace.png')
plt.close('all')
# -------------------------------------------------------------------------------------------------------------
# loss functions
ae_loss_weight = 1.
...
def train_step(batch_x):
    # Autoencoder
    # TODO: Generieren von Daten im Latentspace
    with tf.GradientTape() as ae_tape:
        encoder_output = encoder(batch_x, training=True)
        # Generate z-data
        noise = tf.random.normal([batch_x.shape[0], z_dim], mean=2, stddev=0.1)
        # --> Shape (256, 2)
        new_input = tf.concat((encoder_output, noise), axis=0)
        # --> Shape (512, 2)

        # # Autoencoder loss: Half mse
        # decoder_output_shuffle = tf.gather(decoder_output, tf.random.shuffle(tf.range(tf.shape(decoder_output)[0])))
        # new_decoder_output = decoder_output_shuffle[decoder_output_shuffle.shape[0]//2]
        #
        # ae_loss = autoencoder_loss(batch_x, new_decoder_output, ae_loss_weight)
...
for epoch in range(n_epochs):
    ...
    if epoch % 100 == 0:
        ...
        plt.savefig(mean_std_dir / ('epoch_%d.png' % epoch))
        plt.close('all')
        ...
        if epoch == n_epochs - 1:
            ...
            plt.savefig(mean_std_dir / 'validation_latentspace.png')
            plt.close('all')
